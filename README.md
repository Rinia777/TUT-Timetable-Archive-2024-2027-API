...
`{年度}`は `2020`, `2021`, `2022`, `2023` のいずれかを指定してください。
...
## データ更新
このリポジトリは 2024-2027 年度のアーカイブ API 用です。データ取得は GitHub Actions により、年一回、毎年3月1日 JST 3:40 に実行されます。

定期実行では 2024, 2025, 2026, 2027 年度だけを対象にします。対象年度が現在年度に該当する場合は取得をスキップします。指定された4年間分の講義コードと講義詳細 JSON がすべて存在する場合、以後の定期実行では追加取得を行いません。

講義データ更新後に、年度別の学部別サーチインデックスを `docs/api/v1/archive/{年度}/search-index/{学部名}.json` に生成します。曜日・時限・`regularOrIntensive`・教員名・開講時期・対象学年・科目区分の絞り込み用キーはサーチインデックスに含まれます。
